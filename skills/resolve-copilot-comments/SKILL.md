...
description: Addresses unresolved GitHub Copilot review comments on the current pull request. Use when the user asks to "resolve Copilot comments," "address Copilot feedback," "go through the Copilot review," or types `/resolve-copilot-comments`.
---
...
Commit the changes with a conventional commit message describing the fixes, then push to the remote. (Equivalent to running the `commit` skill, then pushing.)
...
Request a new Copilot review via `gh`. If the command succeeds, confirm it to the user. If the CLI does not support it, report that and stop.
